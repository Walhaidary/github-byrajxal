...
import { AdminReceiptActions } from './AdminReceiptActions';
...
export function AdminReceiptList({ receipts, loading, error, onReceiptsUpdated }: ListProps) {
                  ...
                  <tr key={receipt.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex space-x-2">
                        {/* Approval checkbox - only enabled for approved receipts */}
                        <input
                          type="checkbox"
                          checked={selectedForApproval.includes(receipt.id)}
                          onChange={(e) => handleSelectForApproval(receipt.id, e.target.checked)}
                          disabled={!isApproved} // Only allow selection of approved receipts for reversal
                          className={`rounded border-gray-300 text-blue-600 focus:ring-blue-500 
                            ${!isApproved ? 'opacity-50 cursor-not-allowed' : ''}`}
                        />
                        {/* Payment checkbox - only enabled for approved and paid receipts */}
                        <input
                          type="checkbox"
                          checked={selectedForPayment.includes(receipt.id)}
                          onChange={(e) => handleSelectForPayment(receipt.id, e.target.checked)}
                          disabled={!isApproved || !isPaid} // Only allow selection of paid receipts for reversal
                          className={`rounded border-gray-300 text-green-600 focus:ring-green-500 
                            ${(!isApproved || !isPaid) ? 'opacity-50 cursor-not-allowed' : ''}`}
                        />
                      </div>
                    </td>
